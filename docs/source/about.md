---
myst:
    html_meta:
        "description": "About Spondan Bandyopadhyay — data scientist, ML practitioner, curious builder."
        "keywords": "about, Spondan Bandyopadhyay, data science, machine learning, experience"
layout: landing
hide-toc: true
hide-navigation: true
orphan: true
---

# About Me

> *Curious by default. Building things to find out how they work.*

I’m **Spondan Bandyopadhyay** — an AI practitioner focused on understanding 
and building intelligent systems.

What draws me to artificial intelligence is not just the technology itself, 
but its ability to model complex patterns, make decisions, and solve meaningful 
real-world problems. I’m particularly interested in areas such as computer vision, 
reinforcement learning, and generative models.

My journey into this field began with a simple house price prediction case study 
introduced in class. Seeing a mathematical model uncover structure in messy, real-world 
data sparked my curiosity and led me to explore how learning systems actually work.
Since then, I’ve been developing a strong foundation in core concepts — from data exploration 
and feature engineering to model design and evaluation — while gradually moving toward 
more advanced topics in machine learning and AI systems.

I approach learning by combining theory with hands-on implementation, building projects 
and experimenting with ideas to better understand how intelligent systems perceive, learn, and act.

---

## 🛠 Skills & Tools
```{grid} 1 1 2 3
:gutter: 3

:::{grid-item-card} 🤖 ML & Data Science
:class-header: sd-bg-primary sd-text-white

{bdg-primary}`Python`
{bdg-primary}`NumPy`
{bdg-primary}`Pandas`
{bdg-primary}`scikit-learn`
{bdg-primary}`Matplotlib`
{bdg-primary}`Seaborn`
{bdg-primary}`OpenCV`
{bdg-primary}`MediaPipe`
{bdg-primary}`Jupyter`
{bdg-primary}`Excel`
:::

:::{grid-item-card} 🗄 Databases & Infrastructure
:class-header: sd-bg-secondary sd-text-white

{bdg-secondary}`MySQL`
{bdg-secondary}`MongoDB`
{bdg-secondary}`AWS`
{bdg-secondary}`Git`
{bdg-secondary}`GitHub`
:::

:::{grid-item-card} 🧰 Languages & Frameworks
:class-header: sd-bg-dark sd-text-white

{bdg-dark}`Java`
{bdg-dark}`C / C++`
{bdg-dark}`React`
{bdg-dark}`HTML`
{bdg-dark}`CSS`
{bdg-dark}`Tailwind`
{bdg-dark}`Tkinter`
{bdg-dark}`p5.js`
{bdg-dark}`MkDocs`
{bdg-dark}`Sphinx`
:::
```
---

## 💼 Work Experience

```{raw} html
<div class="timeline">

  <div class="tl-item">
    <div class="tl-card">
      <div class="tl-role">Customer Operations Analyst Intern</div>
      <div class="tl-company">Forma AI</div>
      <div class="tl-duration">May 2024 – Ongoing</div>
      <p>Working at the intersection of business strategy, data analysis,
      and AI-driven solutions. Transforming complex data into actionable
      insights, generating reports, and helping clients optimise operations
      using Forma's platform.</p>
      <p>Strengthening expertise in data analysis, business intelligence,
      and cross-functional collaboration.</p>
      <div class="tl-tags">
        <span class="tl-tag">Data Analysis</span>
        <span class="tl-tag">Business Intelligence</span>
        <span class="tl-tag">AI</span>
        <span class="tl-tag">Reporting</span>
      </div>
    </div>
  </div>

  <div class="tl-item">
    <div class="tl-card">
      <div class="tl-role">Project Trainee</div>
      <div class="tl-company">Hexaware Technologies</div>
      <div class="tl-duration">June 2023 – August 2023</div>
      <p>Built a full-stack shopping cart application with a customer
      storefront and admin dashboard, backed by a live database.
      Worked across React, Node.js, and MongoDB — covering database
      integration, UI design, and user authentication.</p>
      <div class="tl-tags">
        <span class="tl-tag">React</span>
        <span class="tl-tag">Node.js</span>
        <span class="tl-tag">MongoDB</span>
        <span class="tl-tag">Full Stack</span>
      </div>
    </div>
  </div>

</div>
```
---

## 📬 Get in Touch

I'm always happy to talk about data, ML ideas, or interesting projects.

```{grid} 1 2 2 3
:gutter: 3

:::{grid-item-card} 🐙 GitHub
:link: https://github.com/SpondanB
:link-type: url

See what I'm building.
:::

:::{grid-item-card} 💼 LinkedIn
:link: https://www.linkedin.com/in/spondan-bandyopadhyay/
:link-type: url

Let's connect professionally.
:::

:::{grid-item-card} 📧 Email
:link: mailto:spondan2003@gmail.com
:link-type: url

Drop me a message directly.
:::
```

---

```{admonition} A note on this site
:class: note

This site is built with [Sphinx](https://www.sphinx-doc.org/) using the
[Shibuya theme](https://shibuya.lepture.com/) and
[MyST-Parser](https://myst-parser.readthedocs.io/). It doubles as a
portfolio and a personal notebook — projects and notes live here side by side.
```